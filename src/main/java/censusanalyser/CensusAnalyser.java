    ...
    private void sort(Comparator<IndiaCensusDTO> censusCSVComparator) {
        for (int i = 0; i< censusList.size(); i++){
            for(int j = 0; j< censusList.size()-1-i; j++){
                IndiaCensusDTO indiaCensusDTO = censusList.get(j);
                IndiaCensusDTO indiaCensusDTO1 = censusList.get(j+1);
                if(censusCSVComparator.compare(indiaCensusDTO, indiaCensusDTO1) > 0){
                    censusList.set(j,indiaCensusDTO1);
                    censusList.set(j+1,indiaCensusDTO);
                }
            }
        }
    }
